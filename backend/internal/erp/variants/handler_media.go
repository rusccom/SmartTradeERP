...
func (h *Handler) DeleteMedia(w http.ResponseWriter, r *http.Request) {
	tenantID := tenant.FromContext(r.Context())
	err := h.service.DeleteMedia(r.Context(), tenantID, r.PathValue("id"), r.PathValue("mediaID"))
	if err != nil {
		h.writeMediaError(w, err)
		return
	}
...
func (h *Handler) SetPrimaryMedia(w http.ResponseWriter, r *http.Request) {
	tenantID := tenant.FromContext(r.Context())
	err := h.service.SetPrimaryMedia(r.Context(), tenantID, r.PathValue("id"), r.PathValue("mediaID"))
	if err != nil {
		h.writeMediaError(w, err)
		return
	}
