...
func (h *Handler) FullStock(w http.ResponseWriter, r *http.Request) {
	tenantID := tenant.FromContext(r.Context())
	query := fullStockListQuery(r)
	rows, total, err := h.service.FullStock(r.Context(), tenantID, query)
	if err != nil {
		httpx.WriteError(w, http.StatusInternalServerError, "internal_error", "failed to load full stock", err.Error())
		return
	}
	meta := &httpx.Meta{Page: query.Page, PerPage: query.PerPage, Total: total}
	httpx.WriteData(w, http.StatusOK, rows, meta)
}
...
func fullStockListQuery(r *http.Request) httpx.ListQuery {
	sort := httpx.SortConfig{
		Allowed:  []string{"name", "sku_code", "global_qty", "avg"},
		Fallback: "name",
	}
	return httpx.ParseListQuery(r, sort, []string{"product_id", "variant_id", "warehouse_id"})
}
